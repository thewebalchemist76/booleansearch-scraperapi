...
function App() {
  ...
  const handleSearch = async () => {
    ...
    if (totalSearches > 50) {
      const confirmed = window.confirm(
        ...
        `Tempo stimato: ${Math.round(totalSearches * 3 / 60)} minuti.\n\n` +
        `Vuoi continuare?`
      )
      if (!confirmed) {
        setIsSearching(false)
  ...
  const downloadCSV = () => {
    ...
    link.download = `ricerche_google_${new Date().toISOString().split('T')[0]}.csv`
    link.click()
  }
    ...
    <div className="app">
      <div className="container">
        <header className="header">
          <h1>🔍 Boolean Search - Google via ScraperAPI</h1>
          <p className="subtitle">Cerca articoli su più domini con ricerca booleana Google</p>
          <p className="info-text">⚡ Ogni ricerca richiede ~2-3 secondi • Powered by ScraperAPI</p>
        </header>
